<?php
class Rdm_Jobs_Invoice_Helpers{
	...
	*/
	public static function get_invoices_for_client_extra_columns($clientid,$return_or_show='return'){
		...
		if($clientid && ( $clientid > 0)){
			...
			$invoicesAssociateWithClient = __('No Invoices','rdm-job-manager');
	...
	*/
	...
	static function get_all($options=array()){
		...
		$query = array('posts_per_page' => -1, 'post_type' => 'rdm_invoice');
	...
	*/
	static function get_invoice_notes_value_by_invoice_id($id,$meta=''){
		...
		$meta_value_array = get_post_meta($id,'_rdm_invoice_notes',true);
	...
	*/
	...
	static function dropdown_paid_status(){
		...
		if(isset($_POST['rdm_invoice_status_for_report_page']) && $_POST['rdm_invoice_status_for_report_page']){
			$selected = $_POST['rdm_invoice_status_for_report_page'];
		}
		
		?><select  name="rdm_invoice_status_for_report_page">
	...
	*/
	...
	static function get_results_for_report(){
		...
		if(isset($_POST['rdm_invoice_status_for_report_page']) && $_POST['rdm_invoice_status_for_report_page']!=''){
			...
			$prep_status = array(
							'key' => '_rdm_invoice_notes' , 
							'value' => serialize(strval($_POST['rdm_invoice_status_for_report_page'])) , 
							'compare' => 'LIKE'
							); 
		...
		$invoices_found = sizeof($results_all_invoices) ; 	 
	 
		
	 
		echo  apply_filters('rdm_reports_invoices_page_found_invoices_title' , sprintf( _n( '<h3>Found  %s invoice </h3>', '<h3>Found  %s invoices </h3>', $invoices_found, 'rdm-job-manager' ), $invoices_found ));
		...
		if( $invoices_found >=1 ){
			...
			foreach ($results_all_invoices as $single_invoice){ 
				...
				<tr class=" hentry  iedit <?php echo $color ;?> widefat">
					...
					<td class="check-column" style="padding:9px 0px 8px 10px;">
						<?php 
							$client_id_for_invoice = self::get_invoice_meta_value_by_invoice_id($single_invoice->ID,'rdm_jobs_invoices_client_field_id');
