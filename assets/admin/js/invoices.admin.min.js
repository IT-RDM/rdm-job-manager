jQuery( document ).ready( function ( $ )
{
	...
	} ), jQuery( "input#rdm_apply_vat_btn" ).click( function ()
	{
		...
		jQuery.isNumeric( discountValueEntered ) || ( discountValueEntered = 0 ), jQuery.isNumeric( vatValueEntered ) || ( vatValueEntered = 0 ), rdm_jobs_functions.disable_invoice_buttons(), jQuery.ajax(
		{
			url: ajaxurl,
			type: "POST",
			dataType: "json",
			...
			success: function ( data )
			{
				"yes" == data.invoiceUpdated && ( jQuery( "#PersonTableContainer" ).jtable( "load" ), jQuery( "#rdm_preview_invoice" ).trigger( "click" ) ), "NoItemsOnInvoice" == data.invoiceUpdated && ( alert( albwppm.table_invoice_no_items_on_invoice ), rdm_jobs_functions.disable_invoice_buttons() ), jQuery( ".rdm_apply_vat_loader" ).css( "display", "none" ), rdm_jobs_functions.enable_invoice_buttons()
			},
			error: function () {}
		} )
