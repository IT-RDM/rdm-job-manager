<?php
class Rdm_Jobs_Clients_Helpers{
	...
	public static function get_invoices_for_client_extra_columns($clientid,$return_or_show='return'){
		...
		if($clientid && ( $clientid > 0)){
			...
			$get_invoices_for_clients_params =array(
				'showposts'=>-1,
				'post_type' => 'rdm_invoice',
				'post_status' => 'publish',
				'meta_key'=>'rdm_jobs_invoices_client_field_id',
				'meta_value'=> $clientid
			);
			...
			if(sizeof($results_invoices_for_client)>=1){
				...
				foreach($results_invoices_for_client as $single_invoice_for_client){
					...
					//Invoice Status
					if(get_post_meta($single_invoice_for_client->ID , '_rdm_invoice_notes', true)){
	...
	*/
	static function get_all_as_dropdown(){
		...
		if(self::get_all() > 0 ){
			...
			//get all clients
			$query = array('posts_per_page' => -1, 'post_type' => 'rdm_client');
	...
	static function get_paid(){
		...
		if(sizeof($results_invoices)>=1){
			...
			foreach($results_invoices as $single_invoice){
				...
				//Invoice Status
				if(get_post_meta($single_invoice->ID , '_rdm_invoice_notes', true)){
					$invoiceStatus = get_post_meta($single_invoice->ID , '_rdm_invoice_notes', true);
	...
	*/
	static function create_input($input_name,$placeholder=''){
		...
		if(isset($_POST['rdm_client_'.$input_name.'_field_id']) && $_POST['rdm_client_'.$input_name.'_field_id']!=''){
			$selected_value = $_POST['rdm_client_'.$input_name.'_field_id'];
		}	

		echo '<input type="text" name="rdm_client_'.$input_name.'_field_id" placeholder="'.$placeholder.'" value="'.$selected_value.'">';
	...
	*/
	static function get_results_for_report(){
		...
		if(isset($_POST['rdm_client_first_name_field_id']) && $_POST['rdm_client_first_name_field_id']!=''){

			$prep_client_name = array(
							'key' => 'rdm_client_first_name_field_id' , 
							'value' => $_POST['rdm_client_first_name_field_id'] , 
							'compare' => 'LIKE'
							); 
		...
		if(isset($_POST['rdm_client_last_name_field_id']) && $_POST['rdm_client_last_name_field_id']!=''){

			$prep_client_surname = array(
							'key' => 'rdm_client_last_name_field_id' , 
							'value' => $_POST['rdm_client_last_name_field_id'] , 
							'compare' => 'LIKE'
							); 
		...
		if(isset($_POST['rdm_client_email_field_id']) && $_POST['rdm_client_email_field_id']!=''){

			$prep_client_email = array(
							'key' => 'rdm_client_email_field_id' , 
							'value' => $_POST['rdm_client_email_field_id'] , 
							'compare' => 'LIKE'
							); 
		...
		if(isset($_POST['rdm_client_phone_field_id']) && $_POST['rdm_client_phone_field_id']!=''){

			$prep_client_phone = array(
							'key' => 'rdm_client_phone_field_id' , 
							'value' => $_POST['rdm_client_phone_field_id'] , 
							'compare' => 'LIKE'
							); 
		...
		if(isset($_POST['rdm_client_mobile_field_id']) && $_POST['rdm_client_mobile_field_id']!=''){

			$prep_client_mobile = array(
							'key' => 'rdm_client_mobile_field_id' , 
							'value' => $_POST['rdm_client_mobile_field_id'] , 
							'compare' => 'LIKE'
							); 
		...
		if(isset($_POST['rdm_client_skype_field_id']) && $_POST['rdm_client_skype_field_id']!=''){

			$prep_client_skype = array(
							'key' => 'rdm_client_skype_field_id' , 
							'value' => $_POST['rdm_client_skype_field_id'] , 
							'compare' => 'LIKE'
							); 
		...
		if( $clients_found >= 1 ){
			...
			foreach ($results_all_clients as $single_result){ 
				...
				<tr class=" hentry  iedit <?php echo $color ;?> widefat">
					...
					<td class="post-title page-title column-title">
							<?php echo self::get_client_meta_value_by_client_id($single_result->ID,'rdm_client_first_name_field_id') ;?>
					</td>		

					<td class="post-title page-title column-title">
							<?php echo self::get_client_meta_value_by_client_id($single_result->ID,'rdm_client_last_name_field_id') ;?>
					</td>					

					<td class="post-title page-title column-title">
							<?php echo self::get_client_meta_value_by_client_id($single_result->ID,'rdm_client_email_field_id') ;?>
					</td>	

					<td class="post-title page-title column-title">
							<?php echo self::get_client_meta_value_by_client_id($single_result->ID,'rdm_client_phone_field_id') ;?>
					</td>					
					
					<td class="post-title page-title column-title">
							<?php echo self::get_client_meta_value_by_client_id($single_result->ID,'rdm_client_mobile_field_id') ;?>
					</td>	
					<td class="post-title page-title column-title">
							<?php echo self::get_client_meta_value_by_client_id($single_result->ID,'rdm_client_skype_field_id') ;?>
					</td>					
