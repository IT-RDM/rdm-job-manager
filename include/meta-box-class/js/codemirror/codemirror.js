...
var CodeMirror = (function() {
    ...
    function CodeMirror(place, givenOptions) {
        ...
        var hasFocus;
        try { hasFocus = (targetDocument.activeElement == input); } catch (e) {}
        ...
        var instance = wrapper.CodeMirror = {
            ...
            setOption: function(option, value) {
                ...
                if (option == "mode" || option == "indentUnit") loadMode();
                    ...
                    input.blur(); } else if (option == "readOnly" && !value) { resetInput(true); } else if (option == "theme") themeChanged();
        ...
        function updateLinesNoUndo(from, to, newText, selFrom, selTo) {
            ...
            for (var i = 0, l = work.length; i < l; ++i) {
                var process = work[i];
                if (process < from.line) newWork.push(process);
                else if (process > to.line) newWork.push(process + lendiff);
            }
        ...
        function fastPoll() {
            ...
            function p() {
                ...
                if (!changed && !missed) { missed = true;
                    poll.set(60, p); } else { pollingFast = false;
                    slowPoll(); }
                endOperation();
            }
            poll.set(20, p);
        ...
        function scrollIntoView(x1, y1, x2, y2) {
            ...
            var screen = scroller.clientHeight,
                ...
                scrolled = false,
                result = true;
            if (y1 < screentop) { scroller.scrollTop = Math.max(0, y1 - 2 * lh);
                scrolled = true; } else if (y2 > screentop + screen) { scroller.scrollTop = y2 + lh - screen;
                scrolled = true; }
        ...
        function highlightWorker() {
            ...
            while (work.length) {
                if (!getLine(showingFrom).stateAfter) var process = showingFrom;
                else var process = work.pop();
                if (process >= doc.size) continue;
                var start = findStartLine(process),
                    state = start && getLine(start - 1).stateAfter;
                ...
                doc.iter(i, doc.size, function(line) {
                    ...
                    if (+new Date > end) {
                        ...
                        if (realChange) changes.push({ from: process, to: i + 1 });
                        return (bail = true);
                    }
    ...
    if (document.documentElement.getBoundingClientRect != null) eltOffset = function(node, screen) {
        ...
        try { var box = node.getBoundingClientRect();
            box = { top: box.top, left: box.left }; } catch (e) { box = { top: 0, left: 0 }; }
        if (!screen) {
